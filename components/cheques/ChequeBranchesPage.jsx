...
export default function ChequeBranchesPage({ templateKey, pageTitle }) {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `/api/cheques/branches?templateKey=${encodeURIComponent(templateKey)}`,
          { cache: "no-store" }
        );
  ...
  }, [ready, canUseCheques, templateKey]);
      ...
      {loading ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {branches.map((branch, index) => (
            ...
            >
              <Link
                href={`/cheques/${templateKey}?branch=${encodeURIComponent(branch.branchKey)}`}
