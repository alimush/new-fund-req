...
/** كل مفاتيح الوصولات اللي المستخدم عنده صلاحية عليها */
export function getAllVoucherCompanyKeysForUser(permissions = []) {
  return getVoucherCompaniesForUser(permissions).map((c) => c.key);
}

/** خيارات وصل التخويل — كل الوصولات اللي المخوَّل عنده صلاحية عليها */
export function getVoucherCompanyOptionsForDelegation(
  requestCompanyKey,
  permissions = []
) {
  const companies = getVoucherCompaniesForUser(permissions);
  const defaultKey = resolveVoucherCompanyKeyForUser(requestCompanyKey, permissions);
  const defaultNorm = normCompanyKey(defaultKey);

  return companies.map((cfg) => ({
    key: cfg.key,
    name: cfg.name || cfg.key,
    isDefault: normCompanyKey(cfg.key) === defaultNorm,
  }));
}

/** وصل الخطوة: يستخدم مفتاح التخويل إن وُجد، وإلا حسب صلاحيات المستخدم */
export function resolveVoucherCompanyKeyForStep(
  requestCompanyKey,
  step,
  permissions = []
) {
  const delegatedKey = String(step?.voucherDelegateCompanyKey || "").trim();
  if (delegatedKey) {
    const cfg = findCompanyByKey(delegatedKey);
    return cfg?.key || delegatedKey;
  }
  return resolveVoucherCompanyKeyForUser(requestCompanyKey, permissions);
}
