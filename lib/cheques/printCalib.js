...
export function defaultPrintCalib(template, fields) {
  ...
  const widthMm = numOr(dims.widthMm, 187);
  const heightMm = numOr(dims.heightMm, 92);
  return {
    pageTopMm: 0,
    pageLeftMm: 0,
    widthMm,
...
export function normalizePrintCalib(raw, template, fields) {
  ...
  return {
    pageTopMm: round2(clamp(numOr(raw?.pageTopMm, d.pageTopMm), -20, 120)),
    pageLeftMm: round2(clamp(numOr(raw?.pageLeftMm, d.pageLeftMm), -20, 120)),
    widthMm: round2(clamp(numOr(raw?.widthMm, d.widthMm), 50, 280)),
    heightMm: round2(clamp(numOr(raw?.heightMm, d.heightMm), 30, 200)),
