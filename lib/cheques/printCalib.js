...
/** إزاحة فعلية عند الطباعة = مجموعة + جزء */
export function getEffectiveFieldOffset(calib, key) {
  ...
  if (isDateSpacingKey(key)) {
    ...
    return {
      offsetXmm: round2(g.offsetXmm + ind.offsetXmm),
      offsetYmm: round2(g.offsetYmm + ind.offsetYmm),
    };
  }
  return getStoredFieldOffset(calib, key);
}
...
export function normalizeFieldOffsets(rawOffsets, template, fields) {
  ...
  for (const key of keys) {
    let item = raw[key];
    if (!item) {
      if (key === DATE_GROUP_KEY) item = legacyDate;
