...
/** حجم الصك الفعلي على الشاشة (مم) — مطابق للطباعة */
export function getChequePhysicalSizeStyle(template) {
  const { widthMm, heightMm } = getChequePrintDimensions(template);
  return {
    width: `${widthMm}mm`,
    height: `${heightMm}mm`,
...
export function printFontSizeToPreviewPx(
  ...
  const baseMm = field
    ? fieldFontSizeMm(field, template, widthMm, layoutFontScale)
    : fallbackMm;
...
/** أقل ارتفاع % لصندوق حقل نصي واحد — يمنع قصّ الحروف عند تكبير الخط */
export function minFieldHeightPercentForFont(field, template, layoutFontScale = 100) {
  if (!field || field.type !== "text") return Number(field?.height) || 6;
  const { height: imgH } = getChequeImageSize(template);
  if (!imgH) return Number(field?.height) || 6;

  const scaled = fieldWithLayoutFontScale(field, layoutFontScale);
  const fontPx = fieldDesignFontPx(scaled);
  const linePx = fontPx * 1.5 + 8;
  return Math.min(22, (linePx / imgH) * 100);
}

/** ارتفاع العرض الفعلي = الأكبر بين المحفوظ والحد الأدنى للخط */
export function fieldDisplayHeightPercent(field, template, layoutFontScale = 100) {
  const stored = Number(field?.height) || 6;
  if (field?.type !== "text") return stored;
  const minH = minFieldHeightPercentForFont(field, template, layoutFontScale);
  return Math.max(stored, Math.round(minH * 10) / 10);
}
