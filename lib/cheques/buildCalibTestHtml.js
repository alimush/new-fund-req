import { normalizePrintCalib, normalizeWizardGuideStyle, WIZARD_GUIDE_STYLE_FRAME } from "@/lib/cheques/printCalib";
import {
  chequePrintPageCss,
...
const crosshairCss = `
  ...
  .calib-frame {
    ...
    border: 0.6mm solid #dc2626; pointer-events: none; z-index: 5;
  }
`;

function buildCoordinatesGuideHtml(w, h, copyIndex, copyCount) {
  const marks = [
...
function buildFrameGuideHtml() {
  return `<div class="calib-frame" aria-hidden="true"></div>`;
}

function buildCalibSheetInnerHtml({ w, h, imageUrl, copyIndex, copyCount, guideStyle }) {
  ...
  const style = normalizeWizardGuideStyle(guideStyle);
  const guideHtml =
    style === WIZARD_GUIDE_STYLE_FRAME
      ? buildFrameGuideHtml()
      : buildCoordinatesGuideHtml(w, h, copyIndex, copyCount);
...
/** PDF/HTML تجريبي للمعايرة — إحداثيات REF أو إطار أحمر */
export function buildCalibTestPrintHtml({
  ...
  const guideStyle = calib.wizardGuideStyle;
  ...
  const sheetsHtml = Array.from({ length: copies }, (_, i) => {
    ...
    <div class="cheque-sheet"${copyAttr}>
      <div class="cheque-inner">
        ${buildCalibSheetInnerHtml({ w, h, imageUrl, copyIndex, copyCount: copies, guideStyle })}
      </div>
    </div>`;
  }).join("");
