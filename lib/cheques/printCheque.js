...
import { normalizePrintCalib, resolveWizardPrintCalib } from "@/lib/cheques/printCalib";
...
function perChequeLayoutsFromPayload(rest = {}) {
  return {
    textFieldLayout: rest.textFieldLayout ?? null,
    amountWordsLayout: rest.amountWordsLayout ?? null,
    amountWordsLine2Layout: rest.amountWordsLine2Layout ?? null,
  };
}

function finalizePrintPayload({
  template,
  fields,
  printCalib,
  layoutFontScale,
  ...
  let calib = resolveCalib(template, printCalib, list);
  if (!skipFontSync) {
    calib = syncPrintCalibFontsFromLayout(calib, list, template, layouts);
  }
  return {
    ...rest,
    template,
    fields: list,
    printCalib: calib,
    layoutFontScale: layoutFontScale ?? 100,
  };
}
...
async function preparePrintPayload(payload = {}) {
  ...
  const layouts = perChequeLayoutsFromPayload(rest);
  ...
  if (useProvidedCalib && printCalib) {
    ...
    const finalized = finalizePrintPayload({
      ...rest,
      template,
      templateKey: key,
      fields: list,
      printCalib: normalized,
      layoutFontScale: rest.layoutFontScale ?? 100,
      layouts,
      skipFontSync: true,
    });
    return {
  ...
  if (typeof window !== "undefined" && key && template) {
    ...
    const list = fields.length ? fields : bundle.fields;
    return finalizePrintPayload({
      ...rest,
      template,
      templateKey: key,
      fields: list,
      printCalib: bundle.printCalib,
      layoutFontScale: rest.layoutFontScale ?? bundle.globalFontScale ?? 100,
      layouts,
      resolvedWizardCalib: resolveWizardPrintCalib({
        printCalib: bundle.printCalib,
        wizardPrintCalib: bundle.wizardPrintCalib,
        wizardCalibSource: bundle.wizardCalibSource,
        template,
        fields: list,
        copyCount: bundle.wizardTestCopyCount,
      }),
...
export async function printChequeImageOnly(payload = {}) {
  ...
  try {
    ...
    const calibForImage = resolveWizardPrintCalib({
      printCalib: prepared.printCalib,
      wizardPrintCalib: prepared.wizardPrintCalib,
      wizardCalibSource: prepared.wizardCalibSource,
      template,
