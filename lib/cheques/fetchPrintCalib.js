...
export async function fetchChequePrintBundle(
  ...
  try {
    ...
    if (layoutJson?.success) {
      ...
      const wizardCalibSource = normalizeWizardCalibSource(layoutJson.wizardCalibSource);
    ...
    const printCalib = mergePrintCalibs(layoutCalib, printerCalib, template, layoutFields);
