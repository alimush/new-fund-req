...
import { fieldDisplayHeightPercent, fieldFontSizeMm } from "@/lib/cheques/chequeDesignMetrics";
import {
  fieldOffsetCss,
  slashTransformCss,
  getFieldFontStyle,
...
import { datePartTextAlign } from "@/lib/cheques/dateUtils";
import { resolveSlashPositions } from "@/lib/cheques/dateSlashLayout";
import { fieldWithTextLayout, fieldWithChequePosition, layoutFromField, AMOUNT_WORDS_KEY, AMOUNT_WORDS_LINE2_KEY, amountWordsPrintCalibKey, fieldWithAmountWordsSharedFont, getAmountWordsSharedFont } from "@/lib/cheques/textFieldLayout";
import {
  chequePrintPageCss,
  chequePrintDualSheetPageCss,
...
function fieldBoxStyle(f, template, layoutFontScale = 100) {
  const height = fieldDisplayHeightPercent(f, template, layoutFontScale);
  return [
    `top:${f.top}%`,
    `left:${f.left}%`,
    `width:${f.width}%`,
...
function renderFieldValue(f, values, template, printWidthMm, calib, layoutFontScale) {
  const key = f.key;
  const raw = values?.[key];
  if (raw == null || raw === "") return "";

  const fontStyle = getFieldFontStyle(calib, amountWordsPrintCalibKey(key), f);
  ...
  if (key === "amountWords" || key === "amountWordsLine2") {
    const color = fontStyle.color || "#0f172a";
    return `<span class="amount-words" style="font-size:${fsMm}mm;font-weight:${fontWeight};color:${color}">${esc(raw)}</span>`;
  }

  const fw = `font-weight:${fontWeight};`;
  const colorStyle = `color:${fontStyle.color};`;
  let valClass = "val-text";
  let extra = "text-align:right;direction:rtl;line-height:1.4;";
  if (f.type === "amount" || key === "amountNumeric") {
    valClass = "val-amount";
    extra = "";
  } else if (f.type === "datePart") {
    valClass = "val-date";
    extra = `text-align:${datePartTextAlign(key)};`;
  }
...
function slashFontMm(calib, dateField, template, printWidthMm, layoutFontScale) {
  const fontStyle = getFieldFontStyle(calib, SLASH_GROUP_KEY, dateField);
...
export function buildChequePrintHtml({
  ...
  const calib = normalizePrintCalib(printCalib, template, list);
  const fieldByKey = Object.fromEntries(list.map((f) => [f.key, f]));

  const staticFields = list.filter((f) => !PER_CHEQUE_KEYS.has(f.key) && isPrintField(f));

  const dateField = fieldByKey.dateDay;
  const slashFsMm = slashFontMm(calib, dateField, template, calib.widthMm, layoutFontScale);
  const slashStyle = getFieldFontStyle(calib, SLASH_GROUP_KEY, dateField);
  const slashFw = slashStyle.fontWeight;
  const slashColor = slashStyle.color;

  let slashesHtml = "";
  if (dateShowSlashes) {
    const slashPositions = resolveSlashPositions(list, true);
    for (const slash of slashPositions) {
      const slashKey = slash.key;
      slashesHtml += `<div class="slash" style="top:${slash.top}%;left:${slash.left}%;width:${slash.width}%;height:${slash.height}%;font-size:${slashFsMm}mm;font-weight:${slashFw};color:${slashColor};${slashTransformCss(calib, slashKey)}">/</div>`;
    }
  }
  ...
  const fieldsHtml = staticFields
    .map((f) => {
      ...
      if (!inner) return "";
      return `<div class="field ${f.type || "text"} ${f.key}" style="${fieldBoxStyle(f, template, layoutFontScale)};${fieldOffsetCss(calib, f.key)}">${inner}</div>`;
    })
    .join("");
  ...
  if (textBase && values?.[TEXT_KEY]) {
    ...
    const textWeight = textFont.fontWeight;
    const fsMm =
      fieldFontSizeMm(tf, template, calib.widthMm, layoutFontScale) *
      getPrintFontSizeMultiplier(calib, textFont);
    textHtml = `<div class="field text-block" style="${fieldBoxStyle(tf, template, layoutFontScale)};${fieldOffsetCss(calib, TEXT_KEY)}"><span style="font-size:${fsMm}mm;font-weight:${textWeight};color:${textFont.color};text-align:right;direction:rtl;white-space:pre-wrap;line-height:1.2;">${esc(values[TEXT_KEY])}</span></div>`;
  }
  ...
  function resolveAmountWordsPrintField(fieldBase, layout) {
    ...
    const withShared = fieldWithAmountWordsSharedFont(base, amountWordsLayout, amountWordsLine2Layout);
    if (layout && (layout.fontSize != null || layout.fontWeight != null)) {
      return {
        ...withShared,
        fontSize: shared?.fontSize ?? layout.fontSize ?? withShared.fontSize,
        fontWeight: shared?.fontWeight ?? layout.fontWeight ?? withShared.fontWeight,
      };
    }
    return withShared;
  }
  ...
  function renderPerChequeLineHtml(fieldBase, layout, value, key) {
    ...
    if (!inner) return "";
    return `<div class="field ${f.type || "text"}" style="${fieldBoxStyle(f, template, layoutFontScale)};${fieldOffsetCss(calib, key)}">${inner}</div>`;
  }
  ...
  if (safeImageUrl) {
    const imageSheet = getImageSheetCalib(calib, template, list);
    const bgImgHtml = `<div class="cheque-bg" style="background-image:url('${safeImageUrl}');"></div><img src="${safeImageUrl}" alt="" crossorigin="anonymous" style="position:absolute;width:0;height:0;opacity:0;pointer-events:none;" aria-hidden="true" />`;
...
<head>
  ...
  <style>${chequePrintDualSheetPageCss(calib, imageSheet)}</style>
</head>
<body>
  <div class="page-root">
  <div class="cheque-image-sheet">
    <div class="cheque-inner">
      ${bgImgHtml}
    </div>
  </div>
  <div class="cheque-data-sheet">
    <div class="cheque-inner">
