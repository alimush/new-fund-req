...
import { resolveFieldFontWeight } from "@/lib/cheques/chequeFontSync";
import {
  fieldOffsetCss,
  getFieldFontStyle,
...
function renderFieldValue(
  ...
  const fontWeight = resolveFieldFontWeight(f, fontStyle.fontWeight);
  ...
  if (key === "amountWords" && amountWordsStyle) {
    ...
    return `<span class="amount-words" style="font-size:${amountFsMm || fsMm}mm;font-weight:${fontWeight};padding-top:${st.paddingTopMm}mm;color:${color}">${esc(raw)}</span>`;
  }
...
export function buildChequePrintHtml({
  ...
  const slashFw = resolveFieldFontWeight(dateField, slashStyle.fontWeight);
  ...
  if (textBase && values?.[TEXT_KEY]) {
    ...
    const textWeight = resolveFieldFontWeight(tf, textFont.fontWeight);
    const fsMm =
