...
import { fieldOffsetCss, normalizePrintCalib } from "@/lib/cheques/printCalib";
...
export function buildChequePrintHtml({
  ...
  const slashFontMm = dateField
    ? fieldFontSizeMm(dateField, template, calib.widthMm)
    : 3.2;
  ...
  if (dateShowSlashes) {
    for (let i = 0; i < DATE_ORDER.length - 1; i++) {
      ...
      slashesHtml += `<div class="slash" style="top:${pos.top}%;left:${pos.left}%;width:${pos.width}%;height:${pos.height}%;font-size:${slashFontMm}mm;${fieldOffsetCss(calib, slashKey)}">/</div>`;
    }
  }

  const fieldsHtml = staticFields
    .map((f) => {
      const inner = renderFieldValue(f, values, amountWordsStyle, template, calib.widthMm);
      if (!inner) return "";
  ...
  if (textBase && values?.[TEXT_KEY]) {
    ...
    const fsMm = fieldFontSizeMm(tf, template, calib.widthMm);
    textHtml = `<div class="field text-block" style="${fieldBoxStyle(tf)};${fieldOffsetCss(calib, TEXT_KEY)}"><span style="font-size:${fsMm}mm;font-weight:${tf.fontWeight || 700};text-align:right;direction:rtl;white-space:pre-wrap;line-height:1.2;">${esc(values[TEXT_KEY])}</span></div>`;
  }
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@600;700;800;900&display=block" rel="stylesheet"/>
