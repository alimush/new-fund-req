...
const fieldStyles = `
  ...
  .field.datePart { align-items: center; overflow: hidden; }
  .field.datePart.dateDay { justify-content: flex-end; }
  .field.datePart.dateMonth { justify-content: center; }
  .field.datePart.dateYear { justify-content: flex-start; }
  ...
  .field .val-date { display: block; width: 100%; direction: ltr; }
  .field.dateDay .val-date { text-align: right; }
  .field.dateMonth .val-date { text-align: center; }
  .field.dateYear .val-date { text-align: left; }
...
function sheetPlacementCss(className, calib, extra = "") {
  const {
    pageTopMm,
    pageLeftMm,
    widthMm,
...
/** A4 — صورة الصك ومنطقة البيانات بمواضع مستقلة */
export function chequePrintDualSheetPageCss(dataCalib, imageCalib) {
  const { pageWidthMm, pageHeightMm } = getA4PaperSize();
  const {
    offsetXmm,
    offsetYmm,
  ...
  const sx = scaleX / 100;
  const sy = scaleY / 100;
  const imageSx = imageCalib.scaleX / 100;
  const imageSy = imageCalib.scaleY / 100;
  ...
  return `
    ...
    * {
    ...
    ${sheetPlacementCss("cheque-image-sheet", imageCalib, "z-index: 1;")}
    ${sheetPlacementCss(
      "cheque-data-sheet",
      dataCalib,
      `z-index: 2; font-family: ${CHEQUE_PRINT_FONT_FAMILY}; color: #0f172a; background: transparent !important;`
    )}
    .cheque-data-sheet .cheque-inner,
    .cheque-data-sheet .data-overlay {
      background: transparent !important;
    }
    .cheque-image-sheet .cheque-inner {
