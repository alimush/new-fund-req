...
function captureFrameStyles(renderW, renderH, printCss) {
  return `
    html, body {
      margin: 0 !important;
      padding: 0 !important;
...
async function mountChequeCaptureInIframe(html, renderW, renderH) {
  ...
  const frameHtml = `<!doctype html><html><head><meta charset="utf-8"/><style>${captureFrameStyles(renderW, renderH, printCss)}</style></head><body style="margin:0;padding:0;background:#fff;">${mount.outerHTML}</body></html>`;
...
function measureCaptureSize(target, renderW, renderH) {
  const sheet = target.querySelector(".cheque-sheet") || target;
...
export async function runChequePrintPdf(html, title = "cheque") {
  ...
  try {
    ...
    const { width, height } = measureCaptureSize(
      mounted.target,
      mounted.renderW,
      mounted.renderH
    );
