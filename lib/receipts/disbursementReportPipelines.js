...
function disbursementDateRangeMatch(range) {
  return {
...
/**
 * فلتر التاريخ بعد ربط الوصل — يطابق عمود التاريخ في الواجهة:
 * غير مصروف → createdAt | مصروف → تاريخ الصرف
 */
export function appendReportDateFilter(pipeline, { from, to, mode = "all" }) {
  const range = buildDateRange(from, to);
  if (!range) return pipeline;
  ...
  if (mode === "disbursed") {
    pipeline.push({ $match: disbursementDateRangeMatch(range) });
    return pipeline;
  }
  ...
  pipeline.push({
    $match: {
      $or: [
        {
          $and: [notDisbursedRequestMatch("__v.0"), { createdAt: range }],
        },
        {
          $and: [disbursedRequestMatch("__v.0"), disbursementDateRangeMatch(range)],
        },
      ],
    },
  });
...
export function buildRegularUserReportPipeline({
  ...
  appendReportDateFilter(pipeline, { from, to, mode });
...
export function buildQuickSuggestPipeline({
  ...
  appendReportDateFilter(pipeline, { from, to, mode });
  pipeline.push({ $limit: 28 }, QUICK_SUGGEST_PROJECT);
