...
/** غير مصروف = لا يوجد وصل فعلي في vouchers (حتى لو بقيت حقول قديمة على الخطوة) */
function notDisbursedMatch() {
  return { "__v.0": { $exists: false } };
}

/** مصروف = وصل موجود في vouchers فقط */
function disbursedMatch() {
  return { "__v.0": { $exists: true } };
}
