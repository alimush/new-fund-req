...
export function buildExWorkflowActionEmailHtml({
  ...
  const arabicActionLine =
    a === "approve"
      ? `تمت الموافقة على الخطوة ${actedStepNum} من <b style="color:#f8fafc">${escapeHtml(typeAr)}</b> بواسطة <b style="color:#f8fafc">${escapeHtml(
          actorName || "System"
        )}</b>.`
      : a === "reject"
      ? `تم رفض الخطوة ${actedStepNum} من <b style="color:#f8fafc">${escapeHtml(typeAr)}</b> بواسطة <b style="color:#f8fafc">${escapeHtml(
          actorName || "System"
        )}</b>.`
      : `تم تحديث <b style="color:#f8fafc">${escapeHtml(typeAr)}</b> بواسطة <b style="color:#f8fafc">${escapeHtml(
          actorName || "System"
        )}</b>.`;
  ...
  const routingLine =
    a === "approve"
      ? toUserName
        ? `تم تحويل ${escapeHtml(typeAr)} إلى الموظف التالي: <b style="color:#f8fafc">${escapeHtml(
            toUserName
          )}</b>.<br/>ننتظر الإجراء على الخطوة ${waitingStepNum}.`
        : `تم تحويل ${escapeHtml(typeAr)} إلى الموظف التالي.<br/>ننتظر الإجراء على الخطوة ${waitingStepNum}.`
      : a === "reject"
      ? toUserName
        ? `تم إرجاع ${escapeHtml(typeAr)} إلى الموظف السابق: <b style="color:#f8fafc">${escapeHtml(
            toUserName
          )}</b>.<br/>ننتظر الإجراء على الخطوة ${waitingStepNum}.`
        : `تم إرجاع ${escapeHtml(typeAr)} إلى الموظف السابق.<br/>ننتظر الإجراء على الخطوة ${waitingStepNum}.`
      : "";
      ...
      ">
        ...
        <div style="padding:18px">
          ...
          ">
            ...
            <br/>
            رقم المستند: <b style="color:#f8fafc">${escapeHtml(String(planId || ""))}</b>
            <br/>
